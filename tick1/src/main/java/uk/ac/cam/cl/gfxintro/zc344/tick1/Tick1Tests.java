...
public class Tick1Tests {
    ...
    class IlluminateWrapper {
        ...
        public IlluminateWrapper() {
            ...
            try {
                invoke_target =
                    Renderer.class.getDeclaredMethod("illuminate",
                                                     Scene.class,
                                                     SceneObject.class,
                                                     Vector3.class,
                                                     Vector3.class,
                ...
                test.addPointLight(
                    new PointLight(new Vector3(0), new ColorRGB(0), 0));
                illuminate(test,
                           new Sphere(new Vector3(0), 0, new ColorRGB(0)),
                           new Vector3(0),
                           new Vector3(0),
                           new Vector3(0));
            } catch (NoSuchMethodException e) { failed = true; }
        }

        public ColorRGB illuminate(Scene scene,
                                   SceneObject object,
                                   Vector3 P,
                                   Vector3 N,
                                   Vector3 O) {
            ...
            try {
                result =
                    (ColorRGB)
                        invoke_target.invoke(renderer, scene, object, P, N, O);
    ...
    private void runTestsPart1() {
        ...
        start("sphere intersection: intersect at correct location");
        {
            Sphere s = new Sphere(new Vector3(0), 1, new ColorRGB(0));
            Ray r = new Ray(new Vector3(0, 0, 2), new Vector3(0, 0, -1));
            check(s.intersectionWith(r).getLocation().equals(
                new Vector3(0, 0, 1)));
        }
        finish();
        ...
        start("sphere intersection: compute correct normal");
        {
            Sphere s = new Sphere(new Vector3(0), 1, new ColorRGB(0));
            Ray r = new Ray(new Vector3(0, 0, -2), new Vector3(0, 0, 1));
            check(s.intersectionWith(r).getNormal().equals(
                new Vector3(0, 0, -1)));
        }
        finish();
        ...
        start("illumination: no lighting");
        {
            ...
            Sphere o =
                new Sphere(new Vector3(0, 0, 2), 1, new ColorRGB(1, 0, 0));
            Vector3 N = new Vector3(0, 0, -1);
            Vector3 P = new Vector3(0, 0, 1);
            Vector3 O = new Vector3(0, 0, 0);
            check(wrapper.illuminate(test, o, P, N, O)
                      .equals(new ColorRGB(0, 0, 0)));
        }
        finish();

        start("illumination: multiple lights");
        {
            PointLight l1 = new PointLight(new Vector3(0, 0, 0),
                                           new ColorRGB(1, 0, 0),
                                           4 * Math.PI);
            PointLight l2 = new PointLight(new Vector3(0, 0, 0),
                                           new ColorRGB(0, 1, 0),
                                           4 * Math.PI);
            ...
            Sphere o =
                new Sphere(new Vector3(0, 0, 2), 1, new ColorRGB(1, 1, 1));
            Vector3 N = new Vector3(0, 0, -1);
            Vector3 P = new Vector3(0, 0, 1);
            Vector3 O = new Vector3(0, 0, 0);
            ColorRGB reference = wrapper.illuminate(s1, o, P, N, O)
                                     .add(wrapper.illuminate(s2, o, P, N, O));
        ...
        start("illumination: diffuse lighting");
        {
            ...
            Scene test = new Scene();
            test.addPointLight(new PointLight(new Vector3(1, 0, -1),
                                              new ColorRGB(1, 1, 1),
                                              4 * Math.PI));
            test.setAmbientLight(new ColorRGB(0, 0, 0));
            Sphere o =
                new Sphere(new Vector3(1, 0, 1), 1, new ColorRGB(1, 0, 0));
            Vector3 N = new Vector3(0, 0, -1);
        ...
        start("illumination: specular lighting");
        {
            ...
            Scene test = new Scene();
            test.addPointLight(new PointLight(new Vector3(2, 0, 0),
                                              new ColorRGB(1, 1, 1),
                                              4 * Math.PI));
            test.setAmbientLight(new ColorRGB(0, 0, 0));
            Sphere o =
                new Sphere(new Vector3(1, 0, 1), 1, new ColorRGB(0, 0, 0));
            Vector3 N = new Vector3(0, 0, -1);
            Vector3 P = new Vector3(1, 0, 0);
            Vector3 O = new Vector3(0, 0, 0);
            ColorRGB colour = wrapper.illuminate(test, o, P, N, O);
            check(colour.r == o.getPhong_kS() && colour.r == colour.g &&
                  colour.g == colour.b);
        }
        finish();
        ...
        start("illumination: cos(theta)<0");
        {
            // set up a scene such that there is no diffuse term
            Scene test = new Scene();
            test.addPointLight(new PointLight(new Vector3(2, 0, 4),
                                              new ColorRGB(1, 1, 1),
                                              4 * Math.PI));
            test.setAmbientLight(new ColorRGB(0, 0, 0));
            Sphere o =
    ...
    private void runTestsPart2() {
        ...
        {
            Plane p = new Plane(new Vector3(0, 0, 0.0001),
                                new Vector3(0, 0, -1),
                                new ColorRGB(0));
        ...
        start("plane intersection: intersect at correct location");
        {
            ...
            check(p.intersectionWith(r).getLocation() != null &&
                  p.intersectionWith(r).getLocation().equals(
                      new Vector3(0, 0, 1)));
        }
        finish();
        ...
        start("plane intersection: compute correct normal");
        {
            ...
            check(p.intersectionWith(r).getNormal() != null &&
                  p.intersectionWith(r).getNormal().equals(
                      new Vector3(0, 0, -1)));
        }
        finish();
        ...
        if (!wrapper.failed) {
            ...
            start("illumination: shadows");
            {
                Scene test = new Scene();
                test.setAmbientLight(new ColorRGB(0));
                test.addPointLight(new PointLight(new Vector3(0, 0, 4),
                                                  new ColorRGB(1),
                                                  1000));
                test.addObject(new Plane(new Vector3(0, 0, 3),
                                         new Vector3(0, 0, -1),
                                         new ColorRGB(0)));
                Sphere o =
                    new Sphere(new Vector3(0, 0, 2), 1, new ColorRGB(1, 1, 1));
            ...
            start("illumination: shadow ray with no occluder");
            {
                Scene test = new Scene();
                test.setAmbientLight(new ColorRGB(0));
                test.addPointLight(new PointLight(new Vector3(1, 0, 0),
                                                  new ColorRGB(1),
                                                  1000));
                test.addObject(new Sphere(new Vector3(3, 0, -2),
                                          1,
                                          new ColorRGB(1, 1, 1)));
            ...
            start("tracing: reflections");
            {
                ...
                test.addObject(new Plane(new Vector3(0, 0, -1),
                                         new Vector3(0, 0, 1),
                                         new ColorRGB(1)));
                Sphere o =
                    new Sphere(new Vector3(0, 0, 2), 1, new ColorRGB(0, 0, 0));
                test.addObject(o);
    ...
    public void test(boolean testAll) {
        ...
        if (testAll) {
            ...
            System.err.println(
                "Tested only part 1 code. Pass -a to run all the tests.");
        }
        if (failed) {
