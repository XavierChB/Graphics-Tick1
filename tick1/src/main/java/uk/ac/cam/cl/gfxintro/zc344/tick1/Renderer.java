...
public class Renderer {
    ...
    protected ColorRGB trace(Scene scene, Ray ray, int bouncesLeft) {
        ...
        if (bouncesLeft == 0 || reflectivity == 0) {
            ...
            Vector3 R =
                ray.getDirection().scale(-1.0).reflectIn(N).normalised();
            Ray reflectedRay = new Ray(P.add(N.scale(EPSILON)), R);
            reflectedIllumination =
                this.trace(scene, reflectedRay, bouncesLeft - 1);
     ...
     */
    private ColorRGB illuminate(Scene scene,
                                SceneObject object,
                                Vector3 P,
                                Vector3 N,
                                Vector3 O) {
        ...
        ColorRGB I_a =
            scene.getAmbientLighting();  // Ambient illumination intensity

        ColorRGB C_diff =
            object.getColour();  // Diffuse colour defined by the object
        ...
        for (PointLight light : pointLights) {
            ...
            double distanceToLight =
                light.getPosition().subtract(P).magnitude();
            ...
            if (scene.findClosestIntersection(shadowRay).getDistance() >=
                distanceToLight) {
                ColorRGB diffused =
                    C_diff.scale(k_d).scale(I).scale(Math.max(0.0, N.dot(L)));
                ColorRGB specular = C_spec.scale(k_s).scale(I).scale(
                    Math.pow(Math.max(0.0, R.dot(V)), alpha));
                colourToReturn = colourToReturn.add(diffused).add(specular);
            }
        }
    ...
    public BufferedImage render(Scene scene) {
        ...
        for (int y = 0; y < height; ++y) {
            for (int x = 0; x < width; ++x) {
                ...
                ColorRGB linearRGB = trace(
                    scene,
                    ray,
                    bounces);  // Trace path of cast ray and determine colour
                ColorRGB gammaRGB = tonemap(linearRGB);
                image.setRGB(
                    x,
                    y,
                    gammaRGB.toRGB());  // Set image colour to traced colour
            }
            // Display progress every 10 lines
            if (y % 10 == 9 | y == (height - 1))
                System.out.println(
                    String.format("%.2f", 100 * y / (float)(height - 1)) +
                    "% completed");
        }
        return image;
    }
    ...
    public ColorRGB tonemap(ColorRGB linearRGB) {
        ...
        ColorRGB displayRGB =
            powRGB.scale(powRGB.add(Math.pow(0.5 / a, b)).inv());
